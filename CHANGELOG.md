...
- **Apex is now called "Apex Reimbursement Specialists" everywhere on the site.** Their name was showing up four different ways depending on which page you were on — "APEX" on the Sponsors page, "APEX Reimbursement Specialists" on the agenda, "Apex Reimbursement Specialists" on Deals, and "Apex" on the Live page. Someone browsing from one page to the next could reasonably have thought they were three different companies. All of it now reads the same.
  ...
  - **A bug got fixed along the way.** Gobbie Cohn's speaker card (currently hidden) pointed at a sponsor name that didn't match anything in the sponsor list, so clicking his sponsor logo would have opened nothing at all once he's un-hidden. It matches now.
- **The join pop-up now says "This community is for dentists only"**, instead of just "for dentists", on all ten pages that have it.
- **Rabbi Dr. Ephraim Rudolph's Friday night Oneg & Shiur now links to his bio.** He was the only speaker on the agenda whose name wasn't clickable — you could only find his bio by scrolling the Speakers page.
- **The Sefer Torah page has been taken off the site.** It was live and findable by Google, but nothing on the site linked to it, and it still had placeholder buttons — every dedication button opened an email rather than the real form, and its share image was never made. `dentalwisdom.org/torah` now shows the 404 page. Nothing was deleted: the page is in `_archive/torah/` and can be put straight back when it's ready.
- **Tidied up a big pile of duplicated styling code behind the speaker and sponsor preview pages.** Those 16 private preview pages were each carrying their own copy of the same ~250-400 lines of styling — about 4,900 lines in total, which meant any design change there needed editing sixteen files by hand, and two of them had already drifted out of step. The shared part now lives in two files (`css/review-speaker.css` and `css/review-sponsor.css`) and each page keeps only what's genuinely its own. Down to about 1,100 lines. **Nothing about how those pages look has changed** — all 16 were screenshotted at phone and laptop width before and after, and all 32 images came out pixel-for-pixel identical.
...
- **Dr. Ruth Abramowitz is now live on the conference agenda and the speakers page.** She's the fourth track in the Friday 3:00–5:00 PM hands-on block, alongside Dr. Sam Glick (orthobrain), Dr. Elaine Bylis, and the still-unfilled Adin-sponsored perio track.
  - **Agenda:** "From Palette to Palate: Painting Your Way to Better Hand Skills," Friday 3:00–5:00 PM, 2 CE credits, her name linking through to her speaker card.
  - **Speakers page:** her card sits right after Dr. Bylis in the Friday row — both speak at 3:00 PM — bringing the page to 14 cards. Her photo, the short card text and the full five-paragraph bio are the same words she approved on her preview page.
  - **No sponsor on her session.** Dental Wisdom is buying the supplies, so unlike the Glick and perio tracks hers carries no sponsor credit. That's deliberate, not an omission.
  - **The 15-person cap is not shown on the website.** None of the other hands-on workshops display a limit, so showing one only on hers would look like an oversight on the others. Spots get handled off the site.
  - Checked in a browser: her card and photo render, the bio pop-up opens with all five paragraphs, the agenda shows four tracks in the block, and there are no page errors.
- **`CLAUDE.md` brought up to date.** The speaker count, the Friday roster, the Friday-afternoon block and the "still pending" list all now reflect her. A new section records the decisions behind her session — the title is Ben's confirmation but she was invited to rename it, the 15-person cap and why it's off the website, that Dental Wisdom pays for supplies, and that her preview page and the live page share the same bio text and must be changed together. It also carries forward the one thing she hasn't been told anywhere: that she may be asked to help briefly at the Welcome Party.
  - Two stale facts fixed while in there: the notes claimed Dr. Sara Werb was in the Friday 3:00 PM block (her entry there was archived back in July), and the pending-slots list still described the block as "5 tracks" with two placeholder titles that no longer exist.
