...
- **Music turned on: "TYH Boys — I Wanna Be Like Zusha" now plays under the countdown billboard.** Fades up over 2 seconds, sits at background level so you can talk over it, and fades away across the closing countdown. The track is 3:35 against a 2:00 timer, so it never has to loop — it just stops when the billboard closes.
  - **The file was re-encoded for the web.** Your original was 9.2 MB: a 320 kbps stream plus an embedded album-art image. A web-sized copy — `audio/waiting-room.mp3`, 2.9 MB, audio only — is what the deck actually loads, so the page isn't pulling nine megabytes every time it opens. At background volume the difference is inaudible.
  - **Your original file is untouched and stays on your Mac.** `.gitignore` now covers `liveslides/audio/` with an exception for the web copy, so full-size drops in that folder stay local and only the small version publishes. Drop any future track in the same folder and the same rule applies.
  - **Worth a thought before it goes public:** playing a track on your own call is one thing, but publishing the file to dentalwisdom.org makes it downloadable by anyone who finds the URL — that's distribution rather than performance, and it's a commercial recording. If that's a concern, the alternative is to present the deck from your own machine (where the file works without being published) or to use a track you hold a licence for. Say the word and I'll un-publish the audio while leaving everything else working.
