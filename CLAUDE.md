...
- **Sponsor data** (`js/sponsors-data.js`): fields are `name`, `logoUrl`, `link`, `blurb`, `tier` (platinum/gold/silver/bronze), and `attending` (boolean). Set `attending: true` on any sponsor who will have a booth/table at the conference — this shows a gold "✓ ATTENDING" pill badge on their card (top-left) and in their modal (next to the tier pill). Omit or set `false` for sponsors not physically present.
